...
export const chapter1Levels: Level[] = [
  ...
  {
    id: 16,
    title: "Case File 016 - Critical Server Extraction",
    concept: "AND / OR FILTERS",
    story: "Cyber command states that a back-door was triggered either from servers on the local datacenter, or from high criticality systems. Let's filter our network nodes to locate these vulnerable systems.",
    objective: "Retrieve all columns from 'servers' where 'critical_level' is equal to 'Critical' OR 'location' is equal to 'Datacenter Sector 4'.",
    databaseSchema: [
      {
        name: "servers",
        columns: [
    ...
    hints: [
      "Use the SELECT * FROM servers syntax to start.",
      "Add a WHERE clause specifying two conditions linked by OR.",
      "The condition is: critical_level = 'Critical' OR location = 'Datacenter Sector 4'"
    ],
    initialQuery: "-- Select all highly critical or Sector 4 datacenter servers\nSELECT * FROM servers WHERE ",
    expectedQuery: "SELECT * FROM servers WHERE critical_level = 'Critical' OR location = 'Datacenter Sector 4';"
  },
  {
    id: 17,
    title: "Case File 017 - Giant Data Packets",
    concept: "MULTIPLE AND CLUES",
    story: "An unknown external server has been downloading gigabytes of intelligence. Security filters blocked some, but others succeeded. We need to locate successful network logs exceeding 50,000,000 bytes.",
    objective: "Retrieve the 'source_ip', 'destination_ip', and 'bytes_transferred' columns from 'network_logs' where 'bytes_transferred' is strictly greater than 50000000 AND 'status' is equal to 'Success'.",
    databaseSchema: [
      {
        name: "network_logs",
        columns: [
    ...
    hints: [
      "Select only the three specified columns.",
      "Filter using WHERE bytes_transferred > 50000000 AND status = 'Success'."
    ],
    initialQuery: "-- Filter for successful large transmissions\nSELECT source_ip, destination_ip, bytes_transferred FROM network_logs WHERE ",
    expectedQuery: "SELECT source_ip, destination_ip, bytes_transferred FROM network_logs WHERE bytes_transferred > 50000000 AND status = 'Success';"
  },
  {
    id: 18,
    title: "Case File 018 - Accounts of Interest",
    concept: "IN OPERATOR",
    story: "Our intelligence indicates the syndicate utilizes two primary money laundering accounts: 'ACC-4492' and 'ACC-0012'. We must inspect transactions initiated by either of these accounts.",
    objective: "Retrieve all columns from the 'bank_transactions' table where the 'sender_account' is either 'ACC-4492' or 'ACC-0012' using the IN operator.",
    databaseSchema: [
      {
    ...
    hints: [
      "The IN operator allows you to check if a value matches any value in a list: IN ('val1', 'val2').",
      "Format: WHERE sender_account IN ('ACC-4492', 'ACC-0012')."
    ],
    initialQuery: "-- Retrieve bank logs for suspected syndicates\nSELECT * FROM bank_transactions WHERE ",
    expectedQuery: "SELECT * FROM bank_transactions WHERE sender_account IN ('ACC-4492', 'ACC-0012');"
  },
  {
    id: 19,
    title: "Case File 019 - Clearance Scans",
    concept: "LIKE OPERATOR",
    story: "A physical breach occurred at the main security office. We know the keycard reader recorded a scan on June 28th, 2026. Let's find all scanning entries that took place on that specific day.",
    objective: "Retrieve 'employee_name' and 'last_scanned' from 'access_cards' where the 'last_scanned' timestamp starts with '2026-06-28'.",
    databaseSchema: [
      {
        name: "access_cards",
        columns: [
    ...
    hints: [
      "Use the LIKE operator with a wildcard percent '%' sign.",
      "The pattern is '2026-06-28%'."
    ],
    initialQuery: "-- Retrieve scans on June 28th, 2026\nSELECT employee_name, last_scanned FROM access_cards WHERE ",
    expectedQuery: "SELECT employee_name, last_scanned FROM access_cards WHERE last_scanned LIKE '2026-06-28%';"
  },
  {
    id: 20,
    title: "Case File 020 - Local Subnets",
    concept: "SUBSTR FUNCTION",
    story: "Local network traffic always resides on the '192.168' prefix. We must examine which servers have internal IP structures by taking the first 7 characters of their IP address.",
    objective: "Retrieve the 'ip_address' and 'location' from 'servers' where the first 7 characters of the 'ip_address' are exactly equal to '192.168'.",
    databaseSchema: [
      {
        name: "servers",
        columns: [
    ...
    hints: [
      "Use SUBSTR(ip_address, 1, 7) to get the first 7 characters.",
      "Match this against '192.168' using the equals sign."
    ],
    initialQuery: "-- Find servers operating on local subnets\nSELECT ip_address, location FROM servers WHERE ",
    expectedQuery: "SELECT ip_address, location FROM servers WHERE SUBSTR(ip_address, 1, 7) = '192.168';"
  }
];
